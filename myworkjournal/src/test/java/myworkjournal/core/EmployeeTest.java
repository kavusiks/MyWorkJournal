...
public class EmployeeTest {
  ...
  @BeforeEach public void setUp() {
    ...
    nextMonthWorkPeriod =
        new WorkPeriod(testData.getValidNextMonth(), testData.getValidYear(), testData.getValidHourlyWage());
    ...
    totalWorkHours =
        testData.getWorkNextMonth().getShiftDurationInHours() + testData.getWorkThisMonth().getShiftDurationInHours()
            + testData.getWorkThisMonth2().getShiftDurationInHours() + testData.getWorkThisMonth3()
            .getShiftDurationInHours();
    totalSalary = totalWorkHours * testData.getValidHourlyWage();
    totalShifts = testData.getEmployee().getWorkPeriods().stream()
        .mapToInt(workPeriod -> (workPeriod.getPeriodWorkHistory().size())).sum();
  ...
  public void testGetters() {
    ...
    assertEquals(totalWorkHours, testData.getEmployee().getTotalWorkHours(),
        "The total work hours wasn't as expected!");
    ...
    assertEquals(testData.getThisMonthWorkPeriod(), testData.getEmployee().getBestPaidWorkPeriod(),
        "Wrong best paid workperiod returned.");
    //testing getAverageShiftAmount()
    assertEquals(totalShifts / totalWorkPeriods, testData.getEmployee().getAverageShiftAmount(),
        "Wrong amount of average shift returned.");
    //testing getAverageWorkHours()
    assertEquals(totalWorkHours / totalWorkPeriods, testData.getEmployee().getAverageWorkHours(),
        "Wrong amount of average work hours returned.");
    //testing getAverageSalary()
    assertEquals(totalSalary / totalWorkPeriods, testData.getEmployee().getAverageSalary(),
        "Wrong amount of average salary returned.");
    ...
    assertEquals(testData.getValidHourlyWage(), testData.getEmployee().getAverageHourlyWage(),
        "Wrong amount of hourly wage returned.");
  }

  @Test @DisplayName("testAddWorkPeriod()") public void testAddWorkPeriod() {
    Employee employee = new Employee("Ola");

    assertThrows(IllegalArgumentException.class, () -> employee.addWorkPeriod(null));
    assertTrue(employee.getWorkPeriods().isEmpty());
  ...
  @Test public void testRemoveWorkPeriod() {
    Employee employee = new Employee("Ola");
    WorkPeriod wptest = new WorkPeriod("juni", 2021, 100);

    // testing if it throws exception if workperiod is not there.
    assertThrows(IllegalArgumentException.class, () -> employee.removeWorkPeriod(nextMonthWorkPeriod));

    // testing if it throws exception if workperiod is null
    assertThrows(IllegalArgumentException.class, () -> employee.removeWorkPeriod(null));
