...
public class WorkPeriodTest extends CoreTestData {
	...
	public void testAddWork() {
		...
		assertTrue(thisMonthWorkPeriod.getPeriodWorkHistory().contains(workThisMonth), "Work was not properly added to the WorkPeriod");
		...
		try {
			...
			assertFalse(thisMonthWorkPeriod.getPeriodWorkHistory().contains(workOverNightAtTheLastDayOfMonth), testFailedMessage);
		}
	...
	public void testSetAndGetPeriodWorkHistory() {
		...
		assertEquals(0, thisMonthWorkPeriod.getPeriodWorkHistory().size(), "Verifying that the workPeriod contains no works before test setting workperiodhistory failed");
		...
		assertTrue(thisMonthWorkPeriod.getPeriodWorkHistory().contains(workThisMonth), "The workperiod doesn't contain the correct works that was in the periodworkhistory which was sat for the workperiod..");
		assertTrue(thisMonthWorkPeriod.getPeriodWorkHistory().contains(workThisMonth2), "The workperiod doesn't contain the correct works that was in the periodworkhistory which was sat for the workperiod..");
	...
	public void testGetters() {
		...
		assertEquals(totalWorkHours, thisMonthWorkPeriod.getTotalHours(), "getTotalHours() didn't give the correct amount.");
		//Testing getMonthSalary()
		assertEquals(totalWorkHours*validHourlyWage, thisMonthWorkPeriod.getMonthSalary(), "getMonthSalary() didn't give the correct amount.");
		//Testing getIdentifier()
		String correctIdentifier = validThisMonth+"-"+validYear;
		assertEquals(correctIdentifier, thisMonthWorkPeriod.getIdentifier(), "getIdentifier() didn't give the correct workperiod-identifier string");
		//Testing getPeriodStartDate()
		assertEquals(LocalDate.now().withDayOfMonth(1), thisMonthWorkPeriod.getPeriodStartDate(), "getPeriodStartDate() didn't give the correct startdate for this period");
		//Testing getPeriodEndDate()
		assertEquals(LocalDate.now().withDayOfMonth(LocalDate.now().lengthOfMonth()), thisMonthWorkPeriod.getPeriodEndDate(), "getPeriodEndDate() didn't give the correct enddate for this period");
	}
