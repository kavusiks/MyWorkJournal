...
public class WorkPeriodPersistence implements DataSaver{
  ...
  public WorkPeriod deserialize(Scanner inFile) throws FileNotFoundException {
    ...
    while (inFile.hasNext()) {
      if (inFile.nextLine().equals("WorkPeriod {")) {
        ...
        System.out.println("Måned i wp per" + month);
        year = Integer.parseInt(inFile.nextLine().replace("year:", "").strip());
        System.out.println("år i wp per:" +year);
        //hourlyWage strip??
  ...
  public static void main(String[] args) throws FileNotFoundException {
      ...
      WorkPeriodPersistence wp = new WorkPeriodPersistence("src/main/resources/myworkjournal/persistence/workPeriod.txt", workPeriod);
      wp.writeFile();
      wp.readFile();
